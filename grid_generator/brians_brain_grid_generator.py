...
from automata.brians_brain.brians_brain_automaton import BriansBrainCellState
...
class BriansBrainGridGenerator(GridGenerator):
    ...
    def get_cell_state(self, x: int, y: int) -> BriansBrainCellState:
        if random.uniform(0, 1) <= self.alive_chance:
            return BriansBrainCellState.ALIVE.value
        return BriansBrainCellState.DEAD.value
