...
from automata.binary.binary_automaton import BinaryAutomaton, RandomBinaryAutomaton
...
def random_binary_automaton_from_config(automata: dict, game_size: Tuple[int, int]) -> Callable[[], Automaton]:
    def make_callable(name: str, game_size: Tuple[int, int], grid_configs: dict):
        return lambda: _configged_random_binary_automaton(name, game_size, grid_configs)
...
def _configged_random_binary_automaton(name: str, game_size: Tuple[int, int], grid_configs: dict) -> Automaton:
    grid_generators = []
    for gc in grid_configs:
        initial_size_percentage = gc.get('initial_size_percentage', random.uniform(0.1, 1))
        alive_chance = gc.get('alive_chance', random.uniform(0.3, 0.9)) 
        ...
        gen = BinaryGridGenerator(game_size, initial_size_percentage, (center_x, center_y), alive_chance)
        grid_generators.append(gen)
    automaton = RandomBinaryAutomaton(grid_generators, name, game_size)
    return automaton