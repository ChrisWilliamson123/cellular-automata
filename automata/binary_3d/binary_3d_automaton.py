...
class Binary3dAutomaton(Automaton):
    ...
    def get_next_state(self, _) -> npt.NDArray[np.int8]:
        neighbour_counts = sum(
            np.roll(np.roll(np.roll(self.state, dz, axis=2), dy, axis=0), dx, axis=1) for dy in (-1, 0, 1) for dx in (-1, 0, 1) for dz in (-1, 0, 1) if (dx, dy ,dz) != (0, 0, 0)
        )
